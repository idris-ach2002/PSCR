...
class LinkedBlockingQueue {
    ...
    mutable std::mutex me_;
    std::condition_variable notEmpty;
    mutable std::mutex mf_;
...
void put(E * e) {
    ...
    {
        std::unique_lock<std::mutex> lk(mf_);
    ...
    if (c == 0) notEmpty.notify_one();
    if (c + 1 < capacity) notFull.notify_one();
}
...
E * take() {
    ...
    {
        std::unique_lock<std::mutex> lk(me_);
    ...
    if (c == capacity) notFull.notify_one();
    if (c > 1) notEmpty.notify_one();
    return x;
}
